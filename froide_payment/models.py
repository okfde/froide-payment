...
PAYMENT_METHODS = [
    variant
    for variant in CHECKOUT_PAYMENT_CHOICES
...
class Plan(models.Model):
    ...
    interval = models.PositiveSmallIntegerField(
        verbose_name=_("interval"), choices=MONTHLY_INTERVALS, null=True, blank=True
    )
...
class Subscription(models.Model):
    ...
    def get_absolute_url(self):
        return reverse(
            "froide_payment:subscription-detail", kwargs={"token": str(self.token)}
        )
...
class Order(models.Model):
    ...
    def get_absolute_url(self):
        return reverse("froide_payment:order-detail", kwargs={"token": str(self.token)})

    def get_absolute_payment_url(self, variant):
        return reverse(
            "froide_payment:start-payment",
            kwargs={"token": str(self.token), "variant": variant},
        )
    ...
    def get_success_check_url(self):
        return reverse(
            "froide_payment:order-success", kwargs={"token": str(self.token)}
        )
    ...
    def get_or_create_payment(self, variant, request=None):
        ...
        with transaction.atomic():
            ...
            ).get_or_create(variant=variant, order=self, defaults=defaults)
            if created:
...
class PaymentStatus(BasePaymentStatus):
    ...
    CHOICES = [
        ...
        (BasePaymentStatus.PREAUTH, pgettext_lazy("payment status", "Pre-authorized")),
        (BasePaymentStatus.CONFIRMED, pgettext_lazy("payment status", "Confirmed")),
        (BasePaymentStatus.REJECTED, pgettext_lazy("payment status", "Rejected")),
        (BasePaymentStatus.REFUNDED, pgettext_lazy("payment status", "Refunded")),
